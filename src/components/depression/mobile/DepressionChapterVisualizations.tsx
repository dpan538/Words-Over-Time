...
const FOOTERS: Record<string, string> = {
  ...
  "plateau-detail": "Book phrases do not measure illness prevalence.",
...
function ChartFrame({ chart, children }: { chart: DepressionMobileChart; children: ReactNode }) {
  return (
    <div className={styles.cardLayout} data-depression-card-layout={chart.id}>
      <header className={styles.cardHeader}><h3>{chart.title}</h3><p>{chart.transform}</p></header>
...
function LinePlot({ chart, xTicks, yTicks, yMaximum, annotationYear, threshold, event }: { chart: DepressionMobileChart; xTicks: number[]; yTicks: number[]; yMaximum: number; annotationYear?: number; threshold?: number; event?: [number, number] }) {
    ...
    <div className={styles.chartWithLegend}>
      <ResponsivePlot chart={chart}>{(size) => {
        ...
        return <>
          ...
          {annotationYear ? <><line className={styles.annotationRule} x1={xScale(annotationYear, chart.period, box)} x2={xScale(annotationYear, chart.period, box)} y1={box.top} y2={box.top + box.innerHeight} /><text className={styles.annotationText} x={xScale(annotationYear, chart.period, box) + 5} y={box.top + 13}>sustained lead</text></> : null}
...
function PlateauChart({ chart }: { chart: DepressionMobileChart }) {
  const metrics = chart.metrics ?? [];
  const change = metrics[1] && metrics[0] ? ((metrics[1].value - metrics[0].value) / metrics[0].value) * 100 : 0;
  const difference = metrics[1] && metrics[0] ? metrics[1].value - metrics[0].value : 0;
  return <HorizontalBars chart={chart} maximum={15} ticks={[0, 5, 10, 15]} primary={`${change.toFixed(1).replace("-", "−")}%`} secondary={`${difference.toFixed(2).replace("-", "−")} / M · STAGE MEANS`} />;
}

function ClinicalBeforeRiseChart({ chart }: { chart: DepressionMobileChart }) {
  const stageMean = chart.annotations?.find((annotation) => annotation.detail.includes("stage mean"));
  return <ChartFrame chart={chart}><div className={styles.modernStack}><LinePlot chart={chart} xTicks={[1940, 1960, 1979]} yTicks={[0, .05, .1, .15, .2, .25]} yMaximum={.25} threshold={.1} /><div className={styles.meanReadout}><span><strong>{stageMean?.label.split(" ")[0] ?? "0.006"}</strong>clinical depression · stage mean / M</span></div></div></ChartFrame>;
}

function DiagnosticLabelsChart({ chart }: { chart: DepressionMobileChart }) {
  return <ChartFrame chart={chart}><LinePlot chart={chart} xTicks={[1980, 1983, 2000, 2022]} yTicks={[0, .25, .5, .75, 1]} yMaximum={1} threshold={.1} /></ChartFrame>;
}
  ...
  return (
    <ChartFrame chart={chart}>
      <div className={styles.modernStack}>
        ...
        <div className={styles.meanReadout}>{readouts.map((item) => <span key={item.label}><strong>{item.mean ? value(item.mean) : "—"}</strong>{item.label} mean · <b>{item.endpoint ? value(item.endpoint) : "—"}</b> in 2022</span>)}</div>
      </div>
    </ChartFrame>
  );
}

...
- `after/after-home-panel-two-390x844.jpg`: superseded second Home field containing the removed `null / No route` mark; retain only as pre-fix provenance and do not treat it as final-layout proof.
...
- `home-word-bbox-audit.json`: post-gate measurement schema and source-level geometry contract. All final box, blank-band, clipping, and overflow measurements are explicitly null until the corrected Hub composition can be recaptured in a production browser.
...
- The Home content gate is resolved at source level: the nonexistent `null / No route` mark has been removed, and canonical `study-hub` now supplies the vertical `hub/` link to `/words/hub`. Browser bbox/overflow and native-scale screenshot evidence for this corrected composition remains pending.
...
The remaining browser-evidence gaps keep `HOME_STRUCTURE_ACCEPTED=false` and `READY_FOR_USER_VISUAL_ACCEPTANCE=false`.

The approved-environment `npm run verify` recorded here exited 0 with build ID `Bf-uEuLWPPjL-dPqKBD-1`; that build predates the Hub content-gate correction and must be rerun before final handoff. The preceding sandbox attempt failed only because Turbopack was prohibited from binding an internal worker port. The recorded build generated 31/31 actual working-tree entries, with the route-composition caveat above.
